...
static void not_found(http_request *req, http_response *res) {
  DEBUG("Accessing '%s'. Route not found, returned HTTP_NOT_FOUND: 404",
        req->url);
...
static int match_route(const char *pattern, const char *path,
  ...
  while (ptok && utok) {
    ...
    if (ptok[0] == ':') {
      snprintf(out->params[out->count].key, sizeof(out->params[0].key), "%s",
               ptok + 1);
...
void dispatch_route(int c, http_request *req, struct route routes[], int n) {
  DEBUG("dispatch_route: url='%s', method='%s'", req->url,
        req->method); // ADD THIS
  route_params params;
  ...
  for (int i = 0; i < n; i++) {
    ...
    DEBUG("Route matched: %s %s", routes[i].method, routes[i].pattern);

    // Copy route params to request (optional enhancement)
    // req->route_params = params;

    // Execute middlewares
    int middleware_passed = 1;
    for (int j = 0; j < routes[i].middleware_count; j++) {
      if (!routes[i].middlewares[j](req, &res)) {
        middleware_passed = 0;
        DEBUG("Middleware %d failed", j);
